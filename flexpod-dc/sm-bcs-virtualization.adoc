...
image:sm-bcs-image39.png["Un diagramma che mostra VMware HA e vSphere Metro Storage Cluster tra i siti FlexPod A e B tramite una rete multisito"]
...
image:sm-bcs-image40.png["Un diagramma che mostra i server Cisco UCS B200 M5 e lo storage AFF A250 presso i siti A e B, collegati tramite switch Nexus e un ONTAP Mediator presso il sito C"]
...
image:sm-bcs-image41.png["Un diagramma che mostra vSwitch0, vDS0, gli switch iSCSI-A e iSCSI-B con sei uplink vNIC"]
...
image:sm-bcs-image42.png["Uno screenshot che mostra i gruppi VM/Host per gli host e le VM del sito A e del sito B in vSphere"]
...
image:sm-bcs-image43.png["Uno screenshot della finestra di dialogo Modifica regola VM/Host che configura le VM del Sito A per essere eseguite sugli host del Sito A"]

image:sm-bcs-image44.png["Uno screenshot della finestra di dialogo Modifica regola VM/Host per le VM e gli host del sito B con una policy \"Should run on hosts in group\""]
...
image:sm-bcs-image45.png["Uno screenshot delle opzioni avanzate di vSphere HA con quattro datastore heartbeat e due indirizzi di isolamento configurati"]
...
image:sm-bcs-image46.png["Uno screenshot delle impostazioni di vSphere HA Heartbeat Datastores con quattro datastore specifici selezionati"]

Per ulteriori best practice e configurazioni per VMware HA Cluster e VMware vSphere Metro storage cluster, vedi  https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.avail.doc/GUID-5432CA24-14F1-44E3-87FB-61D937831CF6.html["Creazione e utilizzo di cluster vSphere ha"^], https://core.vmware.com/resource/vmware-vsphere-metro-storage-cluster-vmsc["VMware vSphere Metro Storage Cluster (vMSC)"^] e  https://docs.netapp.com/us-en/netapp-solutions-virtualization/vmware/vmw-vmsc-with-smas.html["NetApp ONTAP con SnapMirror active sync (precedentemente SM-BC) e VMware vSphere Metro Storage Cluster (vMSC)"^].
