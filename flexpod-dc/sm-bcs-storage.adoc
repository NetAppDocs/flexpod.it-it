...
image:sm-bcs-image22.png["Un diagramma che mostra un AFF A250 collegato agli switch Nexus A e Nexus B in due siti"]
...
image:sm-bcs-image23.png["Un diagramma che mostra le porte del controller AFF A250 connesse a Nexus, i gruppi di interfaccia, le porte VLAN e i LIF"]
...
image:sm-bcs-image24.png["Un diagramma che mostra la connettività di avvio SAN iSCSI ridondante dai server B200 M5 ai controller AFF A250"]
...
image:sm-bcs-image25.png["Uno screenshot della finestra di dialogo \"Aggiungi interfacce intercluster\" di ONTAP System Manager con due campi per l'indirizzo IP"]

Per unire i due cluster, completare i seguenti passaggi:

. Generare la passphrase di peering del cluster nel primo cluster.
+
image:sm-bcs-image26.png["Uno screenshot della finestra di dialogo Genera passphrase con le autorizzazioni dei peer del cluster, la validità e una passphrase copiata"]

. Richiamare l'opzione Peer Cluster nel secondo cluster e fornire la passphrase e le informazioni LIF dell'intercluster.
+
image:sm-bcs-image27.png["Uno screenshot che mostra la finestra di dialogo Peer Cluster di ONTAP System Manager con la passphrase e gli indirizzi LIF intercluster"]
...
+
image:sm-bcs-image28.png["Uno screenshot della panoramica Protection Overview di ONTAP System Manager che mostra lo stato del peer del cluster e del mediatore"]
...
image:sm-bcs-image29.png["Uno screenshot della finestra di dialogo Configura Mediator di ONTAP System Manager con i campi di connessione del mediatore"]
...
image:sm-bcs-image30.png["Uno screenshot del riquadro Panoramica della protezione di ONTAP System Manager che mostra lo stato del Mediator configurato"]
...
image:sm-bcs-image31.png["Uno screenshot della finestra di dialogo Protect Consistency Group con volume di origine esxi_a e cluster di destinazione aff-a250-b"]
...
image:sm-bcs-image32.png["Uno screenshot delle relazioni di ONTAP System Manager che mostra due gruppi di coerenza sani e sincronizzati nel sito A"]

Questa schermata mostra le relazioni dei gruppi di coerenza nel sito B.

image:sm-bcs-image33.png["Uno screenshot delle relazioni di ONTAP System Manager nel sito B che mostra due gruppi di coerenza sani e sincronizzati"]

Questa schermata mostra i dettagli delle relazioni del gruppo di coerenza per il gruppo cg_infra_datastore_b.

image:sm-bcs-image34.png["Uno screenshot di ONTAP System Manager che mostra i dettagli della relazione del gruppo di coerenza cg_infra_datastore_b"]
...
image:sm-bcs-image35.png["Uno screenshot delle informazioni sui volumi ONTAP per Infra-SVM-a, inclusi i volumi RW, DP e LS"]

Questa schermata mostra le informazioni sul volume per il sito B.

image:sm-bcs-image36.png["Uno screenshot dell'output del comando ONTAP volume show per Infra-SVM-b che elenca nove volumi, inclusi i volumi DP di destinazione"]
...
image:sm-bcs-image37.png["Uno screenshot che mostra gli igroup iSCSI del sito A e le mappature LUN nell'output della CLI di ONTAP"]

Questa schermata mostra la mappatura di igroups e LUN host per il cluster del sito B.

image:sm-bcs-image38.png["Uno screenshot dell'output del comando di mappatura igroup e LUN del sito B per host VMware iSCSI"]
