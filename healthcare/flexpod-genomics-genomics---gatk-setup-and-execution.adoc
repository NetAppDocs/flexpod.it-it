...
image:flexpod-genomics-image8.png["Un diagramma che mostra un flusso di lavoro genomico dalla raccolta del campione, al sequenziamento, all'analisi delle varianti, all'integrazione con l'EHR e ai modelli predittivi"]
...
+
image:flexpod-genomics-image10.png["Una tabella delle prenotazioni IP VLAN che mostra intervalli, maschere di sottorete e finalità di gestione, VM, NFS e iSCSI"]

. Configurare le LUN di avvio basate su iSCSI sulla SVM ONTAP.
+
image:flexpod-genomics-image9.png["Uno screenshot di ONTAP System Manager che mostra sei LUN di avvio ESXi nella storage VM Healthcare_SVM"]

. Mappare i LUN ai gruppi di iniziatori iSCSI.
+
image:flexpod-genomics-image11.png["Uno screenshot che mostra ESXi_Boot_Lun_1 mappato al gruppo di iSCSI initiator GenomicsESXi_1"]

+
image:flexpod-genomics-image12.png["Uno screenshot di ONTAP System Manager che mostra i dettagli del LUN di avvio di ESXi mappato al gruppo initiator GenomicsESXi_2"]

. Installare vSphere 7.0 con l'avvio iSCSI.
. Registrare gli host ESXi con vCenter.
+
image:flexpod-genomics-image13.png["Uno screenshot del client vSphere che mostra l'inventario con la cartella Genomics, host ESXi e macchine virtuali"]

. Eseguire il provisioning di un datastore NFS `infra_datastore_nfs` Sullo storage ONTAP.
+
image:flexpod-genomics-image14.png["Uno screenshot di ONTAP System Manager che mostra il volume NFS Healthcare_Workload_NFS_Vol selezionato online"]

. Aggiungere il datastore al vCenter.
+
image:flexpod-genomics-image16.png["Uno screenshot del vSphere Client che mostra il riepilogo del datastore NFS 3 infra_datastore_nfs"]

. Utilizzando vCenter, aggiungere un datastore NFS agli host ESXi.
+
image:flexpod-genomics-image15.png["Uno screenshot del vSphere Client che mostra gli host infra_datastore_nfs connessi con stato normale"]
...
+
image:flexpod-genomics-image17.png["Uno screenshot di un terminale Linux che mostra /dev/sdb montato su /mnt/genomics con un utilizzo del 2%"]
