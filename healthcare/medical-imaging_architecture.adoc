...
image:medical-imaging_image3.png["Un diagramma che mostra l'infrastruttura di diagnostica per immagini medicali FlexPod con UCS, Nexus, AFF e switch MDS"]
...
image:medical-imaging_image4.png["Un diagramma che mostra i server SnapCenter, l'integrazione con VMware, i repository di metadati replicati e i plug-in delle applicazioni"]

NetApp MetroCluster configurazioni sono utilizzate da migliaia di aziende in tutto il mondo per l'alta disponibilità (HA), la perdita di dati zero e operazioni non interruttive sia all'interno che all'esterno del data center. MetroCluster è una funzionalità gratuita del software ONTAP che esegue il mirroring sincrono di dati e configurazione tra due cluster ONTAP in posizioni o domini di errore separati. MetroCluster fornisce storage continuamente disponibile per le applicazioni gestendo automaticamente due obiettivi: obiettivo di punto di ripristino (RPO) pari a zero tramite il mirroring sincrono dei dati scritti sul cluster. Obiettivo di tempo di ripristino (RTO) prossimo a zero tramite il mirroring della configurazione e l'automazione dell'accesso ai dati nel secondo sito. MetroCluster offre semplicità con il mirroring automatico di dati e configurazione tra i due cluster indipendenti situati nei due siti. Quando lo storage viene predisposto all'interno di un cluster, viene automaticamente replicato nel secondo cluster nel secondo sito. NetApp SyncMirror technology fornisce una copia completa di tutti i dati con un RPO pari a zero. Pertanto, i carichi di lavoro da un sito possono passare in qualsiasi momento al sito opposto e continuare a fornire dati senza perdita di dati. Ulteriori informazioni sono disponibili  https://fieldportal.netapp.com/s/article/tr4705-netapp-metrocluster-solution-architecture-and-design-1693982882873["qui"^].
...
image:medical-imaging_image5.png["Un diagramma che mostra una clinica di comunità collegata ai sistemi ospedalieri tramite un enterprise service bus"]
...
I sistemi di imaging medicale stanno iniziando a usare  https://www.netapp.com/pdf.html?item=/media/7398-sb-flexpod-datacenter-ai.pdf["Funzionalità di calcolo basate su GPU"^] per ottimizzare i flussi di lavoro umani preelaborando le immagini e migliorando così l'efficienza. I tipici sistemi di imaging medicale aziendali sfruttano le funzionalità di efficienza dello storage leader del settore di NetApp. I sistemi di imaging medicale aziendali in genere usano RMAN per le attività di backup, ripristino e recupero. Per prestazioni migliori e per ridurre il tempo necessario per creare i backup, la tecnologia Snapshot è disponibile per le operazioni di backup e la tecnologia SnapMirror è disponibile per la replica.

La figura seguente mostra i componenti logici dell'applicazione in una vista architetturale a più livelli.

image:medical-imaging_image6.png["Un diagramma che mostra i componenti di imaging medico stratificati su FlexPod con ripristino di emergenza replicato"]

La figura seguente mostra i componenti fisici dell'applicazione.

image:medical-imaging_image7.png["Un diagramma che mostra i data center attivi e di standby FlexPod con storage NetApp ONTAP e server per immagini medicali"]
